...
@st.cache_data
def load_route_data():
    # Load Excel data and rename columns to remove leading/trailing spaces
    df = pd.read_excel("Rute_Perdagangan_Provinsi.xlsx")
    df.columns = df.columns.str.strip()  # Remove any leading/trailing spaces from column names
    df['Provinsi'] = df['Provinsi'].str.strip().str.upper()  # Standardize the 'Provinsi' column
    return df
...
def load_geojson():
    ...
    gdf.columns = gdf.columns.str.strip()  # Remove any leading/trailing spaces from column names
    gdf['Provinsi'] = gdf['Provinsi'].str.strip().str.upper()  # Standardize the 'Provinsi' column
    return gdf
...
if merged.empty:
    st.error("Data gabungan kosong. Pastikan kolom 'Provinsi' di kedua file sudah sesuai.")
else:
    # Map setup
